...
import solutions.seam.SeamCarver;

import java.util.Arrays;

public class ShowSeams {

    public static void showHorizontalSeam(SeamCarver sc) {
        Picture picture = SCUtility.toEnergyPicture(sc);
        Integer[] array = sc.findHorizontalSeam();
        int[] horizontalSeam = Arrays.stream(array).mapToInt(i -> i).toArray();
    ...
    public static void showVerticalSeam(SeamCarver sc) {
        Picture picture = SCUtility.toEnergyPicture(sc);
        Integer[] array = sc.findVerticalSeam();
        int[] verticalSeam = Arrays.stream(array).mapToInt(i -> i).toArray();
