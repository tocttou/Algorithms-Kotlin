...
class MaxPQ<Key : Comparable<Key>> : Iterable<Key> {
    ...
    private fun sink(k: Int) {
        ...
        while (2 * index <= size) {
            ...
            val indexToSwitch = when (j) {
                size -> j
                else -> if (pq[j] as Key > pq[j + 1] as Key) j else j + 1
            }
    ...
    override fun iterator(): Iterator<Key> = MaxPQIterator()
    private inner class MaxPQIterator : Iterator<Key> {
