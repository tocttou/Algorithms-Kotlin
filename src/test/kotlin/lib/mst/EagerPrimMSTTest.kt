...
object EagerPrimMSTTest : Spek({
    describe("an eager prim MST implementation and a weighted undirected graph") {
        val filename = resolve("/mst/undirectedgraph1.txt")
        val `in` = In(filename)
        val edgeWeightedgraph = EdgeWeightedGraph(`in`)
        `in`.close()
        val eagerPrimMST = EagerPrimMST(edgeWeightedgraph)
        context("enumerating the MST's edges") {
            val mstEdges = eagerPrimMST.edges()
        ...
        context("calculating the total weight") {
            val weight = eagerPrimMST.weight()
            it("should be equal to the test value") {
